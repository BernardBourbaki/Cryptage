...
[![Version](https://img.shields.io/badge/version-38.0.6-blue.svg)](https://github.com/BernardBourbaki/Cryptage/releases)
...
* **V37 / V37.1 / V37.2 / V37.2.1 / V37.3 / V37.3.1 / V38.0.0 / V38.0.1 / V38.0.2 / V38.0.3 / V38.0.4 / V38.0.5 / V38.0.6** : Déchiffre **UNIQUEMENT** les fichiers .crypt créés avec V37+
...
1. Téléchargez Cryptage_V38.0.6.exe depuis [Releases](https://github.com/BernardBourbaki/Cryptage/releases/latest)
...
```bash
gcc -finput-charset=UTF-8 -fexec-charset=CP1252 Cryptage_Main.c Cryptage_Core.c Cryptage_UI_Common.c Cryptage_UI.c -o Cryptage_V38.0.6.exe -lssl -lcrypto -lgdi32 -lcomctl32 -mwindows
```
...
* Gardez plusieurs copies du logiciel Cryptage_V38.0.6.exe
...
* 🛡️ **Vérification NULL sur original_extension** : `dup_extension()` (utilisée par `detect_file_type()` pour JPEG/PNG/BMP) peut échouer en cas d'échec d'allocation mémoire critique. Le message de succès d'import (`handle_import`) déréférençait ce pointeur sans contrôle via un `%s` de `snprintf`, comportement indéfini pouvant provoquer un crash sur le runtime C Windows. La longueur associée (`original_extension_len`) n'est désormais renseignée que si l'allocation a réussi.

### Compatibilité

* 🔧 Format .crypt inchangé : compatibilité totale V38.0.5 ↔ V38.0.6
* 🔧 Compilation : MSVC / MinGW-w64 avec OpenSSL 3.2+

## 📊 Nouveautés V38.0.5 (21 août 2026)

### Correction de robustesse post-release

* 🐛 **Gestion correcte de l'échec de bin_to_hex() dans handle_operation_complete** : si l'allocation mémoire échoue lors de la conversion du résultat chiffré ou déchiffré en hexadécimal (`bin_to_hex` retourne `NULL`), l'interface ne passe plus dans un état incohérent. L'état `encrypted`/`decrypted` reste à `FALSE`, la zone « Sortie » est vidée, un message d'erreur explicite est affiché, et les boutons d'export/sauvegarde ne s'activent pas. Auparavant, l'absence de vérification du retour de `bin_to_hex()` pouvait laisser l'application dans un état affirmant un succès sans données valides en sortie.

### Compatibilité

* 🔧 Format .crypt inchangé : compatibilité totale V38.0.4 ↔ V38.0.5
* 🔧 Compilation : MSVC / MinGW-w64 avec OpenSSL 3.2+
