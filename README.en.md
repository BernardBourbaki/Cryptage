...
* **V37 / V37.1 / V37.2 / V37.2.1 / V37.3 / V37.3.1 / V38.0.0 / V38.0.1 / V38.0.2 / V38.0.3 / V38.0.4 / V38.0.5 / V38.0.6 / V38.0.7 / V38.0.8**: Decrypts **ONLY** .crypt files created with V37+
...
1. Download Cryptage_V38.0.8.exe from [Releases](https://github.com/BernardBourbaki/Cryptage/releases/latest)
...
* Keep several copies of the Cryptage_V38.0.8.exe program
...
## 📊 What's New in V38.0.8 (September 12, 2026)

* 🔧 **UI fix**: final cosmetic adjustment — harmonization of the margins (white bands on the right and bottom). The Windows interface is now perfectly balanced.

### Compatibility

* 🔧 Unchanged .crypt format: full compatibility V38.0.7 ↔ V38.0.8
* 🔧 Build: MSVC / MinGW-w64 with OpenSSL 3.2+

## 📊 What's New in V38.0.7 (August 28, 2026)

* 🔧 **Memory fix**: secure_malloc() used to fail on a 0-byte allocation (VirtualAlloc rejects that size), preventing decryption of an empty encrypted text. Fixed by allocating at least 1 byte in that case. Unreachable through the UI (both the text and image paths already block empty input upstream) — found and verified through a new unit-test harness for the cryptographic core.
...
* 🛡️ **NULL check on original_extension**: `dup_extension()` (used by `detect_file_type()` for JPEG/PNG/BMP) may fail on critical memory allocation failure. The import success message (`handle_import`) dereferenced this pointer unchecked via a `%s` in `snprintf`, undefined behavior that could crash on the Windows C runtime. The associated length (`original_extension_len`) is now only set if allocation succeeded.

### Compatibility

* 🔧 Unchanged .crypt format: full compatibility V38.0.5 ↔ V38.0.6
* 🔧 Build: MSVC / MinGW-w64 with OpenSSL 3.2+

## 📊 What's new in V38.0.5 (August 21, 2026)

### Post-release robustness fix

* 🐛 **Correct handling of bin_to_hex() failure in handle_operation_complete**: if memory allocation fails during the conversion of encrypted or decrypted result to hexadecimal (`bin_to_hex` returns `NULL`), the interface no longer enters an inconsistent state. The `encrypted`/`decrypted` state remains `FALSE`, the "Sortie" zone is cleared, an explicit error message is displayed, and the export/save buttons are not activated. Previously, the absence of a return value check for `bin_to_hex()` could leave the application in a state claiming success without valid output data.

### Compatibility

* 🔧 Unchanged .crypt format: full compatibility V38.0.4 ↔ V38.0.5
* 🔧 Build: MSVC / MinGW-w64 with OpenSSL 3.2+
