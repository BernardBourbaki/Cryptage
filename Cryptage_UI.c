...
void create_ui_controls(HWND hwnd, HINSTANCE hInstance, AppContext* ctx) {
    ...
    // ========================================
    // PANNEAU DROIT : 4 groupes avec espaces doublés entre les groupes
    // ========================================
    int rightPanelX = 520;
    int buttonWidth = 270;
    int btnY = y + 20;       // Départ un peu plus bas pour centrage global
    int normalSpacer = 10;   // Espace actuel entre boutons à l'intérieur d'un groupe
    int bigSpacer = 40;      // Espace quadruplé entre les groupes (10 × 4)
    ...
    btnY += 50 + bigSpacer; // espace doublé après le groupe 1
    ...
    btnY += 40 + bigSpacer; // espace doublé après le groupe 2

    // GROUPE 3 : DÉCHIFFRER + EXPORTER texte + EXPORTER image
    ...
    btnY += 40 + bigSpacer; // espace doublé après le groupe 3
    ...
    switch (msg) {
        ...
        case WM_DRAWITEM: {
            ...
            if (p->CtlType == ODT_BUTTON) {
                ...
                // V37.2 : le pinceau créé par CreateSolidBrush est maintenant
                // capturé et détruit après usage - l'ancien code le passait
                // directement à FillRect sans jamais appeler DeleteObject,
                // ce qui fuyait un handle GDI à chaque repaint de bouton.
        ...
        case WM_CLOSE:
            // V37.2 : refus net de fermer pendant une opération en cours,
            // au lieu de demander confirmation puis de fermer quand même.
            // L'ancien comportement laissait secure_mem_cleanup() (dans
            // WM_DESTROY) libérer la mémoire encore utilisée par le thread
            // crypto en arrière-plan - accès à une mémoire déjà libérée,
            // plantage possible. Une opération se termine en quelques
            // secondes ; on demande juste d'attendre.
            if (IS_OPERATION_BUSY(ctx)) {
                show_error(hwnd,
