/**
 * Cryptage_UI.c
 * Interface utilisateur unique - Version 3807
...
void create_ui_controls(HWND hwnd, HINSTANCE hInstance, AppContext* ctx) {
    ...
    // Zone Entrée (V38 : plus de ES_READONLY à la création)
    ...
    ctx->hHelpPanel = CreateWindowA("Static",
        ...
        "Etape 4 - Pour chiffrer du texte saisi : tapez dans Entrée > CHIFFRER\r\n\r\n"
        "Etape 5 - Pour déchiffrer un hex collé : collez dans Entrée > DÉCHIFFRER\r\n\r\n"
...
void update_buttons(AppContext* ctx) {
    ...
    // Vérifier si Entrée contient du texte (pour CHIFFRER en mode texte libre)
    int input_len = GetWindowTextLengthA(ctx->hInputEdit);

    if (!ctx->state.file_imported) {
        // État initial : aucun fichier importé
        // CHIFFRER actif s'il y a du texte dans Entrée
    ...
    } else if (ctx->state.file_type == FILE_TYPE_IMAGE) {
        // Image importée
        ...
        // Fichier texte importé ou texte libre
        SET_BUTTON_STATE(ctx->hEncryptBtn, (input_len > 0) && !ctx->state.encrypted);
        SET_BUTTON_STATE(ctx->hSaveBtn, ctx->state.encrypted);
        SET_BUTTON_STATE(ctx->hDecryptBtn, input_len > 0);
        SET_BUTTON_STATE(ctx->hExportTextBtn,
...
void handle_import(HWND hwnd, AppContext* ctx) {
    ...
    // V37.3.1 : réinitialiser l'extension d'origine entre imports successifs
    ...
    // Vérifier version antérieure (V31-V36)
    if (data_len >= 4) {
    ...
    if (type == FILE_TYPE_TEXT) {
        ...
        SendMessageA(ctx->hInputEdit, EM_SETREADONLY, TRUE, 0);
    }
...
void handle_encrypt(HWND hwnd, AppContext* ctx) {
    ...
    // V38 : pour une image, utiliser loaded_data ; sinon, lire hInputEdit
    if (ctx->state.file_type == FILE_TYPE_IMAGE && ctx->state.loaded_data) {
        text = ctx->state.loaded_data;
        text_len = ctx->state.loaded_len;
        use_loaded_data = TRUE;
    } else {
        // Texte : lire le contenu actuel de la zone Entrée
        char* input_text = secure_get_edit_text(ctx->hInputEdit, hwnd,
            "Erreur Chiffrement", MAX_TEXT_LEN);
        if (!input_text) {
            secure_clean_and_free(password, strlen(password));
            return;
        }
        ...
        if (text_len == 0) {
            ...
            secure_clean_and_free(password, strlen(password));
            secure_free(input_text);
            return;
        }
        text = (unsigned char*)input_text;
    }
    ...
    if (!text_copy) {
        secure_clean_and_free(password, strlen(password));
        if (!use_loaded_data) secure_free(text);
        return;
    }
    memcpy(text_copy, text, text_len);

    if (!use_loaded_data) {
        secure_free(text); // libérer le buffer de secure_get_edit_text
    }
...
void handle_decrypt(HWND hwnd, AppContext* ctx) {
    ...
    // V38 : si un .crypt est importé, utiliser loaded_data ; sinon, lire l'hex de Entrée
    if (ctx->state.file_type == FILE_TYPE_CRYPT && ctx->state.loaded_data) {
        text = ctx->state.loaded_data;
        text_len = ctx->state.loaded_len;
        use_loaded_data = TRUE;
    } else {
        char* hex = secure_get_edit_text(ctx->hInputEdit, hwnd,
            "Erreur Déchiffrement", MAX_TEXT_LEN * 4);
        if (!hex) {
        ...
        if (!is_valid_hex(hex)) {
            show_error(hwnd, "Le contenu de la zone Entrée n'est pas un hexadécimal valide.",
        ...
        if (bin_len < MIN_ENCRYPTED_SIZE) {
            show_error(hwnd, "Données trop courtes pour être un fichier .crypt valide",
                "Erreur Déchiffrement");
            secure_free(bin_data);
            ctx->state.decrypt_attempt_failed = TRUE;
            return;
        ...
        uint32_t stored_mem_kib = read_uint32_le(bin_data + 20);
        if (stored_mem_kib < 4096 || stored_mem_kib > 1048576) {
            show_error(hwnd, "Paramètres de fichier crypté invalides",
                "Erreur Déchiffrement");
            secure_free(bin_data);
            ctx->state.decrypt_attempt_failed = TRUE;
            return;
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DRAWITEM: {
        ...
        if (p->CtlType == ODT_BUTTON) {
            ...
            FillRect(p->hDC, &p->rcItem, hBgBrush);
    ...
    case WM_CLOSE:
        if (IS_OPERATION_BUSY(ctx)) {
            show_error(hwnd,
                "Une opération de chiffrement est en cours.\n"
                "Veuillez patienter quelques secondes qu'elle se termine avant de fermer.",
                "Opération en cours");
            return 0;
        }
        handle_clear(ctx);
